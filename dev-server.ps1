...
# Stop all services
if ($StopAll) {
    Write-Host "🛑 Stopping all FlowSpace services..." -ForegroundColor Red
...
foreach ($name in $services.Keys) {
    ...
    try {
        ...
        if ($recheckPort) {
            Write-Host "[$name] " -NoNewline -ForegroundColor $svc.Color
            Write-Host "✓ Started successfully on port $($svc.Port)" -ForegroundColor Green
        } else {
            Write-Host "[$name] " -NoNewline -ForegroundColor $svc.Color
            Write-Host "⚠️  Started but port not responding yet" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "[$name] " -NoNewline -ForegroundColor $svc.Color
...
if ($failedServices.Count -gt 0) {
    Write-Host "⚠️  Some services couldn't start:" -ForegroundColor Yellow
