...
Write-Host "         Your Teams x Slack x Zoom hybrid platform          " -ForegroundColor Cyan
Write-Host "=============================================================" -ForegroundColor Cyan
Write-Host ""
...
foreach ($check in $checks) {
    ...
    if ($running) {
        ...
        Write-Host "[X] $($check.Name) not responding" -ForegroundColor Red
        $allGood = $false
    }
}
...
if (-not $allGood) {
    Write-Host ""
    Write-Host "        Some services are not ready. Check the dev-server window for errors." -ForegroundColor Yellow
    Write-Host ""
...
if ($backendRunning) {
    ...
    Write-Host "        [!] Backend not responding on port 4000" -ForegroundColor Yellow
}
...
Write-Host "=============================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "[DONE] You should now have:" -ForegroundColor Green
